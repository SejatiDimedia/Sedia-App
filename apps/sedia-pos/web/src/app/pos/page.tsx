...
function POSContent() {
    ...
    // Manager Auth for delete
    const [showManagerAuth, setShowManagerAuth] = useState(false);
    const [pendingDeleteId, setPendingDeleteId] = useState<string | null>(null);
    // Simple role simulation - in production, this would come from session/login
    const [currentRole] = useState<string>("cashier"); // Change to "manager" to test
    // Customer selection
    ...
    const startQrisPolling = (orderId: string) => {
        const interval = setInterval(async () => {
            ...
            if (isPaid) {
                ...
                completeCheckout(orderId, 'paid'); // Save to DB
            }
        }, 3000);

        // Cleanup on unmount or modal close handled via refs or effects?
        // Ideally use useEffect depends on showQrisModal, but let's keep it simple.
        // We'll store interval in a ref if needed, but for now standard var is lost.
        // Actually better to use useEffect in the Modal or Qris component.
        return interval;
    };
    ...
    const handlePrint = () => {
        ...
        const printData = {
            ...lastTransaction,
            outletName: selectedOutletId ? outlets.find(o => o.id === selectedOutletId)?.name : "Sedia POS",
            outletAddress: selectedOutletId ? outlets.find(o => o.id === selectedOutletId)?.address : undefined,
            outletPhone: selectedOutletId ? outlets.find(o => o.id === selectedOutletId)?.phone : undefined,
        };
    ...
    const loadInitialData = async () => {
        ...
        try {
            ...
            if (outletsData.length > 0) {
                const outletId = outletsData[0].id;
                ...
                // Fetch customers for this outlet
                const customersRes = await fetch(`/api/customers?outletId=${outletId}`);
                if (customersRes.ok) {
                ...
                // Fetch active shift
                const shiftRes = await fetch(`/api/shifts?outletId=${outletId}&status=open`);
                if (shiftRes.ok) {
                ...
                // Fetch employees for shift selection
                const employeesRes = await fetch(`/api/employees?outletId=${outletId}`);
                if (employeesRes.ok) {
                ...
                // Fetch member tiers
                const tiersRes = await fetch(`/api/loyalty/tiers?outletId=${outletId}`);
                if (tiersRes.ok) {
                    const tiersData = await tiersRes.ok ? await tiersRes.json() : [];
                    setMemberTiers(tiersData);
                }

                // Fetch payment methods
                const paymentsRes = await fetch(`/api/outlets/${outletId}/payment-methods`);
                if (paymentsRes.ok) {
                    const paymentsData = await paymentsRes.json();
                    setAvailablePaymentMethods(paymentsData);
                }
    ...
    const getTax = () => {
        if (!taxSettings || !taxSettings.is_enabled) return 0;
    ...
    const addPayment = (methodId: string) => {
        ...
        const newPayment = {
            ...
            bank: methodId === 'transfer' ? selectedBank || 'bca' : undefined, // Store selected bank or default
        };
    ...
    const handleCheckout = async () => {
        ...
        if (activePaymentMethod === 'qris' || activePaymentMethod === 'transfer') {
            ...
            try {
                ...
                const payload: any = {
                    ...
                    paymentType: activePaymentMethod === 'qris' ? 'qris' : 'bank_transfer'
                };
                ...
                if (activePaymentMethod === 'transfer') {
                    // Check for VA number in response (usually in va_numbers array or permata_va_number)
    ...
    const completeCheckout = async (invoiceNumber: string, pStatus: string) => {
        setIsCheckingOut(true);
        try {
            const transactionData = {
                ...
                paymentMethod: appliedPayments.length > 1 ? "Split" : selectedPayment,
                paymentStatus: pStatus,
                status: "completed",
                midtransId: (selectedPayment === 'qris' || selectedPayment === 'transfer') ? invoiceNumber : undefined,
        ...
        <div className="flex h-screen flex-col bg-gradient-to-br from-primary-50 via-white to-secondary-50">
                    ...
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className={`ml-4 flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition-all shadow-sm active:scale-95 ${activeShift
                                    ? 'bg-[#fef7e6] text-[#c28f09] hover:bg-[#fdf0cc] border border-[#fbe8b6]'
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <p className="text-sm font-medium text-primary-500">
                                {activeShift
                                    ? `Kasir: ${employees.find(e => e.id === activeShift.employeeId)?.name || '...'}`
                                    : 'Shift Belum Dibuka'
                                }
                            </p>
                        </div>
            ...
            <div className="flex flex-1 gap-4 overflow-hidden p-4">
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8">
                                {filteredProducts.map((product) => (
                                    <button
                                        ...
                                        className="group flex flex-col rounded-xl border border-primary-100 bg-white p-4 text-left shadow-sm transition-all hover:border-secondary-300 hover:shadow-lg hover:shadow-secondary-100/50 active:scale-95"
                                    >
                                        <div className="mb-3 flex h-20 w-full items-center justify-center rounded-lg bg-gradient-to-br from-primary-50 to-secondary-50">
                                            <Package className="h-10 w-10 text-primary-300 group-hover:text-secondary-500" />
                                        </div>
                ...
                <div className="flex w-96 flex-col overflow-hidden rounded-2xl border border-secondary-100 bg-white shadow-lg shadow-secondary-100/50 xl:w-[420px]">
                    {/* Cart Header */}
                    <div className="flex items-center justify-between border-b border-secondary-100 bg-gradient-to-r from-secondary-500 to-secondary-400 p-3">
                        <div className="flex items-center gap-2">
                            <ShoppingCart className="h-5 w-5 text-secondary-900" />
                            <h2 className="text-base font-bold text-secondary-900">Keranjang</h2>
                            <span className="rounded-full bg-secondary-900/20 px-2 py-0.5 text-xs font-bold text-secondary-900">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="flex items-center gap-1.5 rounded-lg border border-secondary-600 bg-secondary-400 px-2 py-1 text-[11px] font-bold text-secondary-900 transition-colors hover:bg-secondary-300"
                            >
                                <Package className="h-3.5 w-3.5" />
                                Ditahan
                                {heldOrders.length > 0 && (
                                    <span className="rounded-full bg-secondary-900 px-1 py-0.5 text-[9px] font-bold text-secondary-100">
                                        {heldOrders.length}
                                    </span>
                                )}
                            </button>
                            {cart.length > 0 && (
                                <button onClick={clearCart} className="text-xs text-secondary-900/70 hover:text-secondary-900 px-1">
                                    Reset
                                </button>
                            )}
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4">
                        {cart.length === 0 ? (
                            ...
                            <div className="space-y-2">
                                {cart.map((item) => (
                                    <div key={item.id} className="flex gap-2 rounded-lg border border-primary-100 bg-primary-50/50 p-2">
                                        <div className="flex-1 min-w-0">
                                            <p className="font-semibold text-primary-900 text-sm truncate">{item.name}</p>
                    ...
                    <div className="flex flex-col border-t border-primary-100 bg-gradient-to-r from-primary-50 to-secondary-50 overflow-hidden max-h-[60%] shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                            ...
                            <div className="mb-3 space-y-1.5">
                                ...
                                <div className="flex justify-between border-t border-primary-200 pt-2 text-xl font-black">
                                    ...
                                    <span className="text-secondary-600">{formatPrice(getTotal())}</span>
                                </div>
                            </div>
                            ...
                            <div className="mb-4">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {(availablePaymentMethods.length > 0 ? availablePaymentMethods : [
                                        ...
                                        const Icon = method.type === 'qris' ? QrCode : method.type === 'transfer' ? CreditCard : Banknote;
                                        const id = method.id;
                                                ...
                                                <div key={idx} className="group relative overflow-hidden rounded-2xl bg-white p-3 border border-primary-100 shadow-sm transition-all hover:border-secondary-300">
                                                    <div className="flex items-center justify-between mb-2">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-xs font-bold uppercase tracking-tight text-primary-600">{p.paymentMethod}</span>
                                                        </div>
                                ...
                                {/* Bank Selection for Transfer (Main selection) */}
                        ...
                        <div className="border-t border-primary-100 bg-white p-4">
                            {/* Summarized Mini Total if scrolled */}
                            <div className="mb-2 flex justify-between items-center px-1">
                                <span className="text-[10px] font-bold text-primary-400 uppercase tracking-widest">Sisa Tagihan</span>
                                <span className={`text-xs font-black ${getRemainingBalance() === 0 ? 'text-green-600' : 'text-secondary-600'}`}>
                                    {formatPrice(Math.abs(getRemainingBalance()))}
                                </span>
                            </div>
                        </div>
                        ...
                        <button
                            ...
                            disabled={cart.length === 0 || isCheckingOut || (selectedPayment === 'transfer' && !selectedBank) || !activeShift || (isSplitMode && getRemainingBalance() !== 0)}
                ...
                showCheckoutModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-primary-900/30 backdrop-blur-sm">
                        <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
                            ...
                            <div className="mb-6 space-y-3 rounded-xl bg-gradient-to-r from-primary-50 to-secondary-50 p-4">
                                <div className="flex justify-between text-sm">
                                    <span className="text-primary-600">Total Item</span>
                                    <span className="font-medium text-primary-900">{cart.reduce((sum, i) => sum + i.quantity, 0)}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-primary-600">Metode Pembayaran</span>
                                    <span className="font-medium capitalize text-primary-900">{selectedPayment}</span>
                                </div>
                                {getTax() > 0 && (
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className="font-medium text-primary-900">{formatPrice(getTax())}</span>
                                    </div>
                                )}
                                <div className="flex justify-between border-t border-primary-200 pt-3 text-2xl font-bold">
                                    <span className="text-primary-900">Tagihan</span>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    ...
                                    className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-secondary-500 to-secondary-400 py-3 font-bold text-secondary-900 hover:from-secondary-400 hover:to-secondary-300 disabled:opacity-50"
                                >
                                ...
                                <div className="mb-6 rounded-xl bg-zinc-50 p-4 text-left space-y-2 border border-zinc-100">
                                    ...
                                    {lastTransaction.payments.map((p: any, i: number) => (
                                        <div key={i} className="flex justify-between text-sm">
                                            <span className="capitalize text-zinc-600">{p.paymentMethod}</span>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                        <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl text-center">
                            <h3 className="text-xl font-bold text-primary-900 mb-2">
                                {vaNumber ? `Virtual Account ${selectedBank?.toUpperCase()}` : 'Scan QRIS'}
                            </h3>
                            <p className="text-zinc-500 mb-4">
                                {vaNumber ? 'Lakukan transfer ke nomor berikut' : 'Scan QR Code untuk membayar'}
                            </p>

                            <div className="mx-auto mb-6 bg-zinc-50 p-4 rounded-xl border border-zinc-200 inline-block w-full">
                                {vaNumber ? (
                                    <div className="py-2">
                                        <p className="text-sm font-medium text-zinc-500 mb-2">Nomor VA {selectedBank?.toUpperCase()}</p>
                                        <div className="flex items-center justify-center gap-2 mb-3 bg-white p-3 rounded-lg border border-zinc-200 shadow-sm">
                                            <p className="text-2xl font-bold tracking-wider text-primary-700 font-mono">{vaNumber}</p>
                                            <button
                                                onClick={() => {
                                                    navigator.clipboard.writeText(vaNumber);
                                                    alert("Copied!");
                                                }}
                                                className="rounded p-2 text-primary-400 hover:bg-primary-50 hover:text-primary-600 transition-all"
                                    ...
                                    qrisString ? (
                                        <div className="bg-white p-2 rounded-lg inline-block">
                                            <QRCodeSVG value={qrisString} size={220} />
                                        </div>
                                    ) : <Loader2 className="h-10 w-10 animate-spin mx-auto text-primary-500" />
                                )}
                            </div>

                            {/* Debug Info for QRIS Only */}
                            {!vaNumber && debugData?.actions?.find((a: any) => a.name === 'generate-qr-code')?.url && (
                                <div className="mb-4 rounded-lg bg-zinc-100 p-3 max-w-sm mx-auto text-left border border-zinc-200">
                                    <p className="mb-1 text-xs font-medium text-zinc-500">QR Simulator URL:</p>
                                    <div className="flex items-center gap-2">
                                        <code className="block flex-1 overflow-hidden text-ellipsis whitespace-nowrap rounded border border-zinc-300 bg-white p-1.5 text-[10px] text-blue-600 underline">
                                            {debugData.actions.find((a: any) => a.name === 'generate-qr-code').url}
                                        </code>
                                        <button
                                            onClick={() => {
                                                const url = debugData.actions.find((a: any) => a.name === 'generate-qr-code').url;
                                                navigator.clipboard.writeText(url);
                                                alert("Copied URL!");
                                            }}
                                            className="rounded p-1.5 hover:bg-zinc-200 text-zinc-500"
                                            title="Copy URL"
                                        >
                            ...
                            <p className="text-2xl font-bold text-primary-900 mb-4">
                                {formatPrice(appliedPayments.find(p => p.paymentMethod === (vaNumber ? 'transfer' : 'qris'))?.amount || getTotal())}
                            </p>

                            <div className="flex items-center justify-center gap-2 mb-6 rounded-full bg-yellow-50 px-4 py-2 text-yellow-700">
                            ...
                            <button
                                ...
                                className="w-full rounded-xl bg-zinc-100 py-3 font-medium text-zinc-900 hover:bg-zinc-200"
                            >
                                Batal
                            </button>
                        </div>
            ...
            {/* Tutup Shift Modal */}
            {
                showCloseShiftModal && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
                        <div className="w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl animate-in zoom-in-95 duration-200">
                            <div className="flex items-center justify-between bg-amber-600 p-6 text-white">
                                <div>
                                    <h3 className="text-xl font-bold">Tutup Shift & Rekonsiliasi</h3>
                                    <p className="mt-1 text-sm text-amber-100">Hitung uang fisik di laci kasir</p>
                                </div>
                                <button onClick={() => setShowCloseShiftModal(false)} className="rounded-full bg-white/20 p-2 hover:bg-white/30">
                                    <X className="h-5 w-5" />
                                </button>
                            </div>
                            <div className="p-8">
                                ...
                                <button
                                    ...
                                    className="mt-8 flex w-full items-center justify-center gap-2 rounded-2xl bg-amber-600 py-4 text-lg font-bold text-white transition-all hover:bg-amber-700 active:scale-95 disabled:opacity-50"
                                >
